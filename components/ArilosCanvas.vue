...
const initParticles = () => {
    ...
    const verticalMargin = particleRadius * 8 // Aumentado para más espacio vertical
    const usableHeight = VISIBLE_HEIGHT - (verticalMargin * 2)
    const rowSpacing = usableHeight / (NUM_ROWS - 1)

    // Calcular el espacio horizontal utilizable
    const usableWidth = CANVAS_WIDTH - (HORIZONTAL_MARGIN * 2)
    const colSpacing = usableWidth / (NUM_COLS - 1)

    for (let i = 0; i < NUM_ROWS; i++) {
        for (let j = 0; j < NUM_COLS; j++) {
            // Ajustar la posición x para incluir el margen horizontal
            const x = HORIZONTAL_MARGIN + (j * colSpacing)
